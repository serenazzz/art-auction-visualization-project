---
title: "Exploratory Data Analysis and Visualization for Art Auction Data"
...
auc_desc: description of the auction (messy text field - not used for our analysis). <br/>  sale_id: unique auction sale id assigned by Sothebys.
...
The complete preprocessing python notebook file can be found here: https://github.com/serenazzz/art-auction-visualization-project/blob/master/Preprocessing.ipynb
...
This map allows us to see that the missing values for some features like hammer_price_bp, condition, and birth_year are actually only missing at a high rate in certain locations. 
...
There does not seem to be very strong correlation between the two variables. Just a subtle suggestion that auctions with smaller average price tend to have more NAs. Therefore, we only consider location as the main determinant for missing values around hammer_price_bp. 
...
2. take a note to exclude Doha and Dubai fro hammer_price_bp analysis due to low base high NAs
...
General questions about the auction information, understanding fluctuations in lot prices and its correlation with different variables 
...
```{r}
##Hammer Price
ggplot(art_final, aes(x= hammer_price_bp_usd)) + geom_histogram(aes(y=..density..))  + geom_density(col="red") + xlim(0,1500000) + theme_minimal() +xlab("Price") +ylab("Density") +ggtitle("Hammer Price Distribution")+ theme(plot.title = element_text(hjust = 0.5,face="bold"))+scale_y_continuous(labels = scales::comma)
...
p1 <- ggplot(MyData5, aes(x= location, y = B/1000000)) + 
    geom_bar( stat='identity', color="yellow", fill="grey")+labs(y = "Auction Revenue ($M)")+labs(x = "Location") + ggtitle("Auction revenue by location") + theme_minimal() + theme(plot.title = element_text(hjust = 0.5,face="bold"))+scale_y_continuous(labels = scales::comma)
...
p2<-ggplot(MyData_1, aes(x= auc_year, y = B/1000000)) + 
    geom_bar(stat='identity', color="green", fill="grey")+labs(y = "Auction Revenue ($M)")+labs(x = "Year") + ggtitle("Auction revenue by year") + theme_minimal()  + theme(plot.title = element_text(hjust = 0.5,face="bold"))+scale_y_continuous(labels = scales::comma)
...
ggplot(MyData_2b, aes(x= location, y = B/1000000)) + 
    geom_bar(stat='identity', color="blue", fill="grey")+labs(y = "Average price ($M)")+labs(x = "Location") + ggtitle("Average price by location") + theme_minimal()  + theme(plot.title = element_text(hjust = 0.5,face="bold"))
...
df1 <- df1 %>% 
  dplyr::mutate(Auction_Season = forcats::fct_relevel(Auction_Season, "summer")) %>%
  dplyr::mutate(Auction_Season = forcats::fct_relevel(Auction_Season, "fall")) %>%
  dplyr::mutate(Auction_Season = forcats::fct_relevel(Auction_Season, "winter"))
...
```{r echo=FALSE}
art_locfin <- art_finance %>% group_by(auc_year, location) %>% summarise(av_revenue = mean(hammer_price_bp_usd)) %>% filter(location %in%  c("HONG KONG","NEW YORK", "PARIS", "MILAN"))
ggplot(art_locfin, aes(x=auc_year, y=as.numeric(av_revenue/1000000))) +geom_line(col="darkblue")+ggtitle("Financial Crisis' Effect on Average Auction Price ($M)")+ylab("Average Price ($M)")+xlab("Year") + facet_wrap(~location, scales ="free_y")+theme_bw()+scale_x_continuous(breaks = seq(min(art_final$auc_year), max(art_final$auc_year),1))+theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
Our interactive visualization is a scatter plot of lot prices over time built with HTML, CSS, Bootstrap and JavaScript; it is hosted on Firebase and can be viewed here:
https://edav-art-viz.firebaseapp.com/
