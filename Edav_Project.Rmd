...
- log scale Alex's scatter plot 
- variable names to be consistent
- relev to # of lots bar plot - for correct order or season
- paralell coord - hammer_price_bp_usd_range
# Introduction
...
# Note that "provenance", "auc_desc", "auction_house_id", "external_image_url", "literature" and "end_date" were dropped
names(art_df)
```
...
1. Since 'Start Date' and 'End Date' are always the same within the dataset, we decided to only use 'Start Date' for the time when the auction occured. Converted the column to DateTime type.
...
To begin the anlaysis, we want to see the titles of expensive lots and plot their size with respect to the prices
```{r}
df1 <- art_final
...
```
In the above plot, "1" indicates the lot has "untitled" in it and "0" other wise. Anlayzing the mosaic plot above, we can see that "untitled" actually result in a relatively lower price as less "untitled" lots are in the range "$500,000+" while more "untitled" lots are in the lowest range. If the higher prices can not explain the common use of "untitled", what can?
We guess that contemprary artists tend to name their arts "untitled". To explroe this, we put together the auction titles where "untitled" works are presente the most and labeled them with art type. 
...
ggplot(data=untitle_ratio, aes(x=reorder(auc_title,ratio), y=ratio,fill=is_contemporary))+geom_bar(stat="identity")+coord_flip()
```
Of the top 20 auctions that have highest "untitled" ratio, we see that contemporary auction is just slighly more than non-contemporary auction. In fact, by looking at auction names of non-contemporary auctions, we realize that smaller pieces of art (such as decoration, furnitures) tend to have "untitled" in their title as well! Perhaps those contribute mostly to the fact that "untitled" work have relatively lower price.
...
```
We notice that for certain era (e.g. 1750), there are no pieces sold. There are also more outliers for authors born 1900 - 1980. However, due to the coplex change in average price over every 10 years, this box plot looks messy overall. Therefore, we decide to only include the scattor plot as that contains the information more straightforward.
...
```
The labels of hammer price range did not turn out as neat in the begeinning, however, after changing the order of the seasons, we obtained the above graph indicating the clear correlations between seasons and the lot prices. Specifically, there are more lots sold in spring and fall and lots are sold relatively higher in the fall.
...
# Executive Summary
INSERT OPENNING
Next, we will present our most imporatn findings alongside with graphs.
## 1. Art pieces created in the 20th centry on average are sold at higher price.
There are more lots sold where their authors are born in the 20th centry, specifically in 1900s,1930s and 1960s. For simplicity, we call them "modern" pieces and call the lots whose authors were born before 1900 "older" pieces. From the graph, we can see that modern pieces have higher prices on average while having larger variance too. One plausible explanation is that the price range and values for contemporary art are more likely to uneven due to the nature of the contemporary art.
```{r}
df3 <- df2 %>% 
...
```
## 3. Lot prices are closely related the season.
...
## 5. Conclusiion about financial crisis
